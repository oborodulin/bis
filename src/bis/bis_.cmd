...
rem ---------------- EOF definitions.cmd ----------------@Echo Off
rem {Copyright}
rem {License}
...
rem для вывода круглых скобок echo должны быть на отдельных строках (в процедурах)
if defined _log_lvl (
...
if /i "%oper_code%" EQU "%RC_ADD%" (
	echo "%oper_code%" "%key_name%" "%value_name%" "%value_value%"
	call :get_reg_value "%oper_code%" "%key_name%" "%value_name%"
...
rem Реализуется в процедуре, так как в условных конструкциях 
rem невозможно получить корректный Errorlevel от Choice 
