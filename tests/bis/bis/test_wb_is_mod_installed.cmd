...
for /f %%i in ("%src_script%") do set src_dir=%%~dpi
for /f %%i in ("%src_script%") do set src_name=%%~ni

set tst_mod_setup_dir=%tst_root_setup_dir%\%tst_pkg_name%\%tst_mod1_name%
rem начина€ от нул€
set tst_mod_bin_dir_cnt=1
set tst_mod_bin_dir1=%tst_root_setup_dir%\%tst_pkg_name%\%tst_mod1_name%\bin1
set tst_mod_bin_dir2=%tst_root_setup_dir%\%tst_pkg_name%\%tst_mod1_name%\bin2
...
rem определ€ем установлен ли уже модуль
call :is_mod_installed "%tst_mod1_name%" "%tst_mod1_ver%"

call :convert_slashes "win" "!mods[%tst_mod1_name%]#SetupDir!" mods[%tst_mod1_name%]#SetupDir
echo "!mods[%tst_mod1_name%]#SetupDir!" "%tst_mod_setup_dir%"
if /i "!mods[%tst_mod1_name%]#SetupDir!" NEQ "%tst_mod_setup_dir%" popd & endlocal & exit /b 10
...
if !mods[%tst_mod1_name%]#BinDirCnt! NEQ %tst_mod_bin_dir_cnt% popd & endlocal & exit /b 12

for /l %%j in (0,1,!mods[%tst_mod1_name%]#BinDirCnt!) do (
	call :get_test_bin_dir %%j
...
endlocal & set %_exec_name:~5%=!tst_mod_bin_dir%bin_dir_no%!
exit /b 0
rem ---------------- EOF test_wb_is_mod_installed.cmd ----------------